...
{
    private const float MinVibrationInterval = 0.025f;
    
    
    private static float ms_LastVibrationTime;
    
    
    public static void Vibrate(long milliseconds = 25)
    {
        if (!SettingsUI.IsVibrationOn) return;
        
        if (Time.unscaledTime - ms_LastVibrationTime < MinVibrationInterval) return;
        
        Vibration.Vibrate(milliseconds);
        ms_LastVibrationTime = Time.unscaledTime;
    }
}
